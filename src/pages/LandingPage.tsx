        ...
        <section id="promo" className="py-12 px-4 bg-black/60 backdrop-blur-md z-10 relative border-y border-red-900/30">
          <div className="max-w-6xl mx-auto space-y-12">
            ...
            <div className={`grid grid-cols-1 ${promos.length > 1 ? 'lg:grid-cols-2' : ''} gap-8`}>
              {promos.map((promo) => (
                <div key={promo.id} className={`border-2 p-8 md:p-12 text-center rounded-sm relative overflow-hidden group flex flex-col justify-center items-center transition-all ${promo.highlight
                  ...
                  <p className={`text-4xl md:text-6xl font-black mb-8 font-syncopate relative z-10 uppercase leading-tight ${promo.highlight ? 'text-red-600 drop-shadow-[0_0_15px_rgba(255,0,0,0.5)]' : 'text-white'
                    }`}>
                    {promo.description}
                  </p>
                  <div className={`inline-block px-8 py-3 border-2 font-bold text-lg rounded-none uppercase tracking-widest relative z-10 bg-black/50 backdrop-blur-sm ${promo.highlight ? 'border-red-600 text-red-600' : 'border-white/20 text-gray-300'
                    }`}>
                    {promo.day}
                  </div>
                </div>
